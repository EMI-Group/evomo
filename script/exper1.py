...
def run(algorithm_name, problem, key, dim, pop_size, n_objs, num_iter=100):
    try:
        algorithm = {
            "MOEAD": MOEAD(lb=jnp.zeros((dim,)), ub=jnp.ones((dim,)), n_objs=n_objs, pop_size=pop_size, problem=problem, key=key, num_generations=100),
            ...
            "TensorHypE": TensorHypE(lb=jnp.zeros((dim,)), ub=jnp.ones((dim,)), n_objs=n_objs, pop_size=pop_size),
            "NSGA3": NSGA3(lb=jnp.zeros((dim,)), ub=jnp.ones((dim,)), n_objs=n_objs, pop_size=pop_size),
            "TensorNSGA3": TensorNSGA3(lb=jnp.zeros((dim,)), ub=jnp.ones((dim,)), n_objs=n_objs, pop_size=pop_size),
        }.get(algorithm_name)
...
if __name__ == "__main__":
    ...
    algorithm_list = ["MOEAD", "TensorMOEAD", "HypE", "TensorHypE", "NSGA3", "TensorNSGA3"]
