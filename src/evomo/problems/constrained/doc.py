...
class DOC2(CMOP):
    ...
    def fn(self, X: torch.Tensor) -> torch.Tensor:
        ...
        g_temp = torch.sum((X[:,11:16] @ c1[:5,:]) * X[:,11:16], dim=1) \
        ...
        d1[:, 0] = torch.relu((f1 - 1 / 8) ** 2 + (f2 - 1 + torch.sqrt(torch.tensor(1 / 8, device=device, dtype=dtype))) ** 2 - 0.15 ** 2)
        d1[:, 1] = torch.relu((f1 - 1 / 2) ** 2 + (f2 - 1 + torch.sqrt(torch.tensor(1 / 2, device=device, dtype=dtype))) ** 2 - 0.15 ** 2)
        d1[:, 2] = torch.relu((f1 - 7 / 8) ** 2 + (f2 - 1 + torch.sqrt(torch.tensor(7 / 8, device=device, dtype=dtype))) ** 2 - 0.15 ** 2)
        ...
        a = torch.tensor([[-16, 2, 0, 1, 0],
                      ...
                      [-1, -2, -3, -2, -1],
                      [1, 2, 3, 4, 5],
                      [1, 1, 1, 1, 1]],dtype=dtype, device=device)

        c1 = torch.tensor([[30, -20, -10, 32, -10],
                       [-20, 39, -6, -31, 32],
                       [-10, -6, 10, -6, -10],
                       [32, -31, -6, 39, -20],
                       [-10, 32, -10, -20, 30]],dtype=dtype, device=device)
        d = torch.tensor([4, 8, 10, 6, 2],dtype=dtype, device=device)
    ...
    def pf(self):
        ...
        mask = (
            (pf[:, 0] < 0.05)
            | ((pf[:, 0] > 0.2202) & (pf[:, 0] < 0.3830))
            | ((pf[:, 0] > 0.6247) & (pf[:, 0] < 0.7440))
        )
        pf = pf[~mask]
        return pf
...
class DOC5(CMOP):
    ...
    def fn(self, X: torch.Tensor) -> torch.Tensor:
        ...
        c1 = torch.relu(-(f1 + f2 - 1))
        c2 = torch.relu(
            -(f1 + f2 - 1 - torch.abs(torch.sin(10 * torch.pi * (f1 - f2 + 1)))))

        c3 = torch.relu((f1 - 0.8) * (f2 - 0.6))

        # Constraints in decision space
        c4 = -X[:, 1] + 35 * X[:, 2] ** 0.6 + 35 * X[:, 3] ** 0.6
        c5 = torch.abs(-300 * X[:, 3] + 7500 * X[:, 5] - 7500 * X[:, 6] - 25 * X[:, 4] * X[:, 5]
                    + 25 * X[:, 4] * X[:, 6] + X[:, 3] * X[:, 4]) - 0.0001
        c6 = torch.abs(100 * X[:, 2] + 155.365 * X[:, 4] + 2500 * X[:, 7] -
    ...
    def pf(self):
        t = torch.cat([
            torch.arange(0, 9, device=self.device),
            torch.arange(16, 21, device=self.device)
        ], dim=0)/20
        # return load_pareto_front_from_file(os.path.join("DOC", f"{self.name.lower()}_M{self.m}_D{self.d}.pf"))
...
class DOC7(CMOP):
    ...
    def fn(self, X: torch.Tensor) -> torch.Tensor:
        X = clamp(X, self.lb, self.ub)
        c1 = torch.tensor([-6.089, -17.164, -34.054, -5.914, -24.721, -14.986, -24.1, -10.708, -26.662, -22.179],device=X.device,dtype=X.dtype)
        X_temp = X[:, 1:11]
    ...
    def pf(self):
        t=torch.cat([torch.linspace(0, 0.45,self.ref_num, device=self.device), torch.arange(11, 21,device=self.device)/20])
        # return load_pareto_front_from_file(os.path.join("DOC", f"{self.name.lower()}_M{self.m}_D{self.d}.pf"))
        return torch.column_stack([t,1-t])
