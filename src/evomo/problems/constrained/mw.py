...
class MW3(MW):
    ...
    def pf(self):
        ...
        while True:
            C = 0.85 - pf[:, 0] - pf[:, 1] + 0.3 * torch.sin(
                0.75 * torch.pi * self.sqrt2 * (pf[:, 1] - pf[:, 0])
            )**2
            invalid = C > 0
            if not invalid.any():
                break
...
class MW4(MW):
    ...
    def pf(self):
        ...
        l = pf[:, [-1]] - pf[:, :-1].sum(dim=1, keepdim=True)
...
class MW6(MW):
    ...
    def fn(self, X: torch.Tensor) -> torch.Tensor:
        ...
        C = (f0**2) / (1.0 + self.LA3(0.15, 6.0, 4.0, 10.0, theta))**2 \
            + (f1**2) / (1.0 + self.LA3(0.75, 6.0, 4.0, 10.0, theta))**2 - 1.0
    ...
    def pf(self):
        ...
        l = torch.cos(6.0 * torch.atan(pf[:, [1]] / pf[:, [0]])**4.0)**10.0
...
class MW8(MW):
    ...
    def fn(self, X: torch.Tensor) -> torch.Tensor:
        ...
        g0 = f_squared - (1.25 - self.LA2(0.5, 6.0, 1.0, 2.0,
                torch.asin(f[:, [-1]] / torch.sqrt(f_squared)))) ** 2
...
class MW12(MW):
    ...
    def fn(self, X: torch.Tensor) -> torch.Tensor:
        ...
        g0 = -1.0 * (1 - 0.625 * f0 - f1 + 0.08 * torch.sin(2 * torch.pi * (f1 - f0 / 1.6))) * \
...
class MW13(MW):
    ...
    def fn(self, X: torch.Tensor) -> torch.Tensor:
        ...
        g0 = -1.0 * (5.0 - (1 + f0 + 0.5 * (f0 ** 2)) - 0.5 * torch.sin(3 * torch.pi * f0) - f1) * \
