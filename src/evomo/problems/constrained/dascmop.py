...
class DASCMOP1(DASCMOP):
    ...
    def constraints(
        self, X: torch.Tensor, g: torch.Tensor, f0: Optional[torch.Tensor] = None, f1: Optional[torch.Tensor] = None
    ):
        a = 20.0
        b = 2.0 * self.eta - 1.0
        ...
        r = 0.5 * self.gamma
        p_k = torch.tensor([[0.0, 1.0, 0.0, 1.0, 2.0, 0.0, 1.0, 2.0, 3.0]], dtype=X.dtype, device=X.device)
        ...
        if f1 is None:
            f1 = 1 - X**2 + g
        c[:, 2:] = (
            ((f0 - p_k) * torch.cos(theta_k) - (f1 - q_k) * torch.sin(theta_k)) ** 2 / a_k2
...
class DASCMOP7(DASCMOP):
    ...
    def constraints(
        self, X: torch.Tensor, g: torch.Tensor, f0: torch.Tensor, f1: torch.Tensor, f2: torch.Tensor
    ) -> torch.Tensor:
        a = 20.0
        b = 2.0 * self.eta - 1.0
