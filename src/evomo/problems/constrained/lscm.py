...
from .sdc import CEC_2006_fitness, CEC_2006_information, Distance_function
...
def PF_function(PopDec: torch.Tensor, G: torch.Tensor, PF_form: int, Q_form: int) -> torch.Tensor:
    ...
    if PF_form == 1:
        ...
        if Q_form == 1:
            ...
            coeff = 1.0 + G + torch.cat([G[:, 1:], zero], 1)
        elif Q_form == 3:
            coeff = 1.0 + G.sum(1, keepdim=True)
        return coeff * C * B
    ...
    if PF_form == 3 and Q_form == 1:
        ...
        Obj[:, -1:] = g * (M - ((X / g) * (1.0 + torch.sin(3.0 * torch.pi * X))).sum(1, keepdim=True))
        return Obj
...
class LSCM(CMOP):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(d=self.d, m=m, n_iq=1, n_eq=0, lb=lb, ub=ub, device=self.device, dtype=self.dtype, **kwargs)
        ...
        for j in range(self.duan):
            self.lu[j], self.high_D_C[j], self.aaa[j], self.optimal_f[j] = CEC_2006_information(
                self.CEC_Problem[j], device=self.device, dtype=self.dtype
            )
    ...
    def variable_information(self, c: torch.Tensor):
        ...
        """
        frac = c / c.sum()
        self.sublen, self.len, self.CV, self.Dis = (
            [None] * self.duan,
            [None] * self.duan,
            [None] * self.duan,
            [None] * self.duan,
        ...
        for j in range(self.duan):
            ...
            self.len[j] = torch.cat(
                [torch.zeros(1, device=c.device, dtype=torch.long), torch.cumsum(self.sublen[j] * self.nk[j], dim=0)], dim=0
            )
    ...
    def variable_linkage(self, X: torch.Tensor) -> torch.Tensor:
        ...
        for j in range(self.duan):
            ...
            if j == 0:
                ...
                prev_end_excl = self.Dis[j - 1]["end"][
                    self.m - 1, self.nk[j - 1] - 1
                ]  # End position index of all subgroups in previous segment
